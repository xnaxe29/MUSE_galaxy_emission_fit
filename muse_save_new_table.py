...
def fitting_function_rev(popt_init, datax, datay, yerror, **kwargs):
    ...
    init_sigma = kwargs.get('vel_disp_init')  # Initial value for Velocity Dispersion
...
muse_revised_result_array = np.zeros([(x2-x1), (y2-y1), (len(center_list_init)+2), (number_of_narrow_components_init+number_of_wide_components_init), 2])

muse_result_array = np.load('muse_complete_fit.npz.npy')
...
for o in range(x1, x2):
    for p in range(y1, y2):
        print (o, p)
        red_chi_squared1 = muse_result_array[o, p, 0, 0]
        red_chi_squared2 = muse_result_array[o, p, 0, 0]
        pfit_curvefit = muse_result_array[o, p, 1:, 0]
        perr_curvefit = muse_result_array[o, p, 1:, 1]
        ...
        muse_revised_result_array[o, p, :, :, 0] = new_fitted_popt_param_array_rev2
        muse_revised_result_array[o, p, :, :, 1] = new_fitted_perr_param_array2


print ('Saving MUSE Results')
np.save('muse_revised_result_array.npz', muse_revised_result_array)
