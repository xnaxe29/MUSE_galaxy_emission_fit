	...
	if (fit_continuum):
		...
		for i in range(len(params_new)):
			if not (bounds_lower[i]<=params_new[i]<=bounds_upper[i]):
				print ('Detected out of bound parameter.')
...
x1, x2, y1, y2 = binning_count, len(y_full)-binning_count, binning_count, len(x_full)-binning_count

#MAKE AN ARRAY WITH ALL UNIQUE BIN NUMBERS
bin_unique = int(((x2-x1)*(y2-y1))/(binning_count*2)**2)
print ("Number of bins detected - ", bin_unique)
bar = IncrementalBar('Countdown', max = bin_unique)


muse_result_array = np.zeros([(x2-x1), (y2-y1), len(pfit_curvefit_init)+1, 2])

idx1 = find_nearest_idx(wave_orig, 7032.14)
idx2 = find_nearest_idx(wave_orig, 7075.00)
...
for o in range(x1, x2):
	for p in range(y1, y2):
		...
		bar.next()
		print ('\n')
		B_array_new = np.nansum(data_original[:, i-binning_count:i+binning_count, j-binning_count:j+binning_count], axis=(1,2)) / ((binning_count*2)**2)
		C_array_new = np.sqrt(np.nansum(err_original[:, i-binning_count:i+binning_count, j-binning_count:j+binning_count], axis=(1,2))) / ((binning_count*2)**2)
		B_array_new = np.nan_to_num(B_array_new, nan=1e-6, posinf=1e-6, neginf=-1e-6)
		C_array_new = np.nan_to_num(C_array_new, nan=1e-6, posinf=1e-6, neginf=1e-6)
		C_array_new[C_array_new<=0.] = 1e-6
		mask2 = np.where(B_array_new[:]=='')
		B_array_new[mask2] = 1e-6
		mask3 = np.where(C_array_new[:]=='')
		C_array_new[mask3] = 1e-6

		#flux_orig = data_original[:, o, p]
		#err_orig = err_original[:, o, p]
		flux_orig = B_array_new
		err_orig = C_array_new
		file1 = [wave_orig, flux_orig, err_orig]
		data_1d = flux_orig[idx1:idx2]
		sides = (np.nanmean(data_1d[0:10]) + np.nanmean(data_1d[-10:-1]))/2.
		all = np.nanmean(data_1d)
		snr = np.nanmedian(flux_orig[idx1:idx2] / err_orig[idx1:idx2])
		wave, flux, err, FWHM_gal, velscale = get_data_from_file(file1, file_type, require_air_to_vaccum, extra_redshift)
		if (sides > all) and (len(wave) == len(flux)) and (len(wave) == len(err)) and (snr>3.):
			count+=1
			#plt.plot(wave, flux)
			#plt.show()
			#quit()
...
file_name_res = str('binned_') + str(int(binning_count)) + str('_muse_complete_fit.npz')
np.save(file_name_res, muse_result_array)
