...
cont_smoothing_par_init = int(initial_guesses['cont_smoothing_par_init'])
cont_allowed_deviation_init = float(initial_guesses['cont_allowed_deviation_init'])
...
def get_bounds(amp_length, number_of_narrow_components, number_of_wide_components, coeff_len, center_list, **kwargs):
	...
	init_vel = kwargs.get('velocity_init', center_init_array)  # Fit Velocity?
	init_sigma = kwargs.get('vel_disp_init', sigma_init_array)  # Fit Velocity?
	...
	for i in range(len(idx_for_fixing_bounds_factor_init_wide_comp)):
		if (amp_ratio_fixed[int(idx_for_fixing_bounds_factor_init_wide_comp[i])]==True):
			...
			amp_bound_min[-(i+1)] = 1e-2
			amp_bound_max[-(i+1)] = 1e2
	...
	for i in range(len(idx_for_fixing_bounds_factor_init_narrow_comp)):
		if (amp_ratio_fixed[int(idx_for_fixing_bounds_factor_init_narrow_comp[i])]==True):
			...
			amp_bound_min[-(i+len(idx_for_fixing_bounds_factor_init_wide_comp)+1)] = 1e-2
			amp_bound_max[-(i+len(idx_for_fixing_bounds_factor_init_wide_comp)+1)] = 1e2
...
def plot_gaus_group_with_cont_rev(wave_array_rev, *popt, **kwargs):
	...
	#flux_sky_rev = flux_sky + cont_array
	#flux_sky_rev = flux_sky
...
def fitting_function_rev(popt_init, datax, datay, yerror, **kwargs):
	...
	init_vel = kwargs.get('velocity_init', center_init_array)  # Initial value for Velocity
	init_sigma = kwargs.get('vel_disp_init', sigma_init_array)  # Initial value for Velocity Dispersion
	...
	#print ('----------------------------')
	#for b in range(len(params_new)):
	#	print (bounds_lower[b], params_new[b], bounds_upper[b])
	#print ('----------------------------')
...
plt.errorbar(wave, flux, yerr=err, color='tab:blue', alpha=0.5, label='Data', zorder=1)
global res
...
#print (center_init_array)
#center_init_array = [315.]
...
plt.errorbar(wave[masked_array], flux[masked_array], yerr=err[masked_array], color='tab:blue', label='Fitted Region', zorder=2)
...
new_array = np.transpose(np.array([pfit_curvefit, perr_curvefit]))
np.savetxt('fit_params.dat', new_array)
print ('Fit results saved')


#wave_fitted, res_fitted, sky_prof = plot_gaus_group_with_cont_rev(wave[masked_array], *pfit_curvefit)
#plt.plot(wave_fitted, sky_prof, 'm--', label='Total Fit', zorder=3)

wave_fitted, res_fitted = plot_gaus_group_with_cont_rev(wave[masked_array], *pfit_curvefit)
plt.plot(wave_fitted, res_fitted, 'r.-', label='Total Fit', zorder=4)


##################################PLOT_SUBCOMPONENTS##################################

wave_array = wave_fitted
center_list = center_list_init  # Centre List
number_of_narrow_components = number_of_narrow_components_init
number_of_wide_components = number_of_wide_components_init
amp_length = len(amp_init_array)
amp_array, center_array, sigma_array, reddening_val_fit, coeff_fit = get_params(pfit_curvefit, number_of_narrow_components, number_of_wide_components, center_list, amp_length)
print ('E(B-V) - ', reddening_val_fit)
amp_array_rev = retrieve_all_amplitude_list_rev2(list(amp_array), number_of_narrow_components_init, number_of_wide_components_init, position_init_narrow_comp, position_init_wide_comp, position_final_narrow_comp, position_final_wide_comp, comments_on_tied, comments_on_Balmer)
if (coeff_fit is None):
	coeff_val_fit = coeff_init_val
else:
...
cont_array_fitted = chebyshev_disp(wave_array, coeff_val_fit)
plt.plot(wave_array, cont_array_fitted, 'c--', label='Continuum without reddening', zorder=5)
reddening_array_fit = [reddening_val_fit, 1.0, 0.0, 0.0]
cont_array_fitted_rev = func_6(wave_array/(1.+redshift_val), cont_array_fitted, *reddening_array_fit)
plt.plot(wave_array, cont_array_fitted_rev, 'g--', label='Continuum', zorder=6)
...
for j in range(len(center_list)):
	...
	if (comments_on_Balmer[j]):
		for k in range(number_of_narrow_components):
			...
			prof_fit_adv = func_6(wave_array/(1.+redshift_val), prof_fit, *reddening_array_fit)
			plt.plot(wave_array, prof_fit_adv)
			plt.axvline(wave_prof(center_array[count], centre), ls='dashed', color='green', alpha=0.5)
			prof_fit = []
			prof_fit_adv = []
			count+=1
			count_amp+=1
		for l in range(number_of_wide_components):
			...
			prof_fit_adv = func_6(wave_array/(1.+redshift_val), prof_fit, *reddening_array_fit)
			plt.plot(wave_array, prof_fit_adv)
			plt.axvline(wave_prof(center_array[count], centre), ls='dashed', color='magenta', alpha=0.5)
			prof_fit = []
			prof_fit_adv = []
			count+=1
			count_amp+=1
