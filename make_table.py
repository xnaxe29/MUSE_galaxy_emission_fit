	...
	for j in range(len(center_list)):
		...
		if (comments_on_Balmer[j]):
			for k in range(number_of_narrow_components):
				new_fitted_popt_param_array_rev[j, count] = amp_array_rev[count_amp]
				count+=1
				count_amp+=1
...
#np.savetxt('test_5.dat', new_array)
#print ('Fit results saved')

new_fitted_popt_param_array = np.zeros([(len(center_list_init)+2), (number_of_narrow_components_init+number_of_wide_components_init)])
new_fitted_popt_param_array[-1,:] = sigma_array
new_fitted_popt_param_array[-2,:] = center_array
new_fitted_popt_param_array_rev = make_table_with_result(wave[masked_array], new_fitted_popt_param_array, *pfit_curvefit)
idx_to_revise_table = new_fitted_popt_param_array[-2,:].argsort()

#for i in range((len(center_list_init)+2)):
#	new_fitted_popt_param_array_rev[i,:] = new_fitted_popt_param_array_rev[i,idx_to_revise_table]

perr_curvefit_new = np.zeros([len(perr_curvefit)])
for i in range(len(perr_curvefit)):
    perr_curvefit_new[i] = log_to_real_err(pfit_curvefit[i], perr_curvefit[i])
...
new_fitted_perr_param_array = np.zeros([(len(center_list_init)+2), (number_of_narrow_components_init+number_of_wide_components_init)])
new_fitted_perr_param_array[-1,:] = sigma_err_array
new_fitted_perr_param_array[-2,:] = center_err_array
new_fitted_perr_param_array = make_table_with_result(wave[masked_array], new_fitted_perr_param_array, *perr_curvefit_new, check_data_type='err')

'''
    final_array_to_save = get_table(new_fitted_popt_param_array_rev, new_fitted_perr_param_array, center_list_init_names, number_of_narrow_components_init, number_of_wide_components_init)
    
    print ('\n')
    print ('-------------------------------------------')
    print(tabulate(final_array_to_save, tablefmt="latex", floatfmt=".3f"))
    print ('-------------------------------------------')
    print ('\n')
'''

new_fitted_popt_param_array_rev2 = new_fitted_popt_param_array_rev
for i in range((len(center_list_init)+2)):
    new_fitted_popt_param_array_rev2[i,:] = new_fitted_popt_param_array_rev[i,idx_to_revise_table]

new_fitted_perr_param_array2 = new_fitted_perr_param_array
for i in range((len(center_list_init)+2)):
    new_fitted_perr_param_array2[i,:] = new_fitted_perr_param_array[i,idx_to_revise_table]

final_array_to_save2 = get_table(new_fitted_popt_param_array_rev2, new_fitted_perr_param_array2, center_list_init_names, number_of_narrow_components_init, number_of_wide_components_init)


print ('\n')
print ('-------------------------------------------')
print(tabulate(final_array_to_save2, tablefmt="latex", floatfmt=".3f"))
print ('-------------------------------------------')
print ('\n')
